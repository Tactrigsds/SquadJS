...
export default class Rcon extends EventEmitter {
  ...
  async execute(body) {
    return new Promise((resolve, reject) => {
      ...
      if (length > 4152) Logger.verbose("RCON", 1, `Error occurred. Oversize, "${length}" > 4152.`);
      else {
  ...
  #badPacket() {
    Logger.verbose("RCON", 1, `Bad packet, clearing buffer: ${this.#bufToHexString(this.stream)} Pending string: ${this.responseString}`);
