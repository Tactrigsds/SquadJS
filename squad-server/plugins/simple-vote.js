...
export default class SimpleVote extends DiscordBasePlugin {
  ...
  async getLayers() {
    ...
    const whitelistedLayers = [];
    if (this.options.whitelist.length !== 0) {
      for (const layer of Layers) {
        for (const word of this.options.whitelist) {
          if (layer.toLowerCase().includes(word.toLowerCase())) {
            whitelistedLayers.push(layer);
            break;
          }
        }
      }
    ...
    for (const layer of this.server.layerHistory.slice(0, this.options.layerHistoryLimit)) {
      ...
      if (this.options.disallowRepeatedInvasion) {
        if (layer.layer.layerid.toLowerCase().includes('invasion')) {
      ...
      Layers = this.filterLayersByPattern(
        Layers,
        this.checkAndRemovePrefix(layer.layer.layerid),
        false
      );
    }
    return Layers;
  ...
  checkAndRemovePrefix(layer) {
    for (const prefix of this.options.layerPrefixes) {
      if (layer.toLowerCase().startsWith(prefix.toLowerCase())) {
        return layer
          .toLowerCase()
          .slice(prefix.length, prefix.length + 6)
          .replace(/_/g, '');
      }
    }
